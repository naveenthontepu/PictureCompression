...
import com.antam.image.compression.ImageCompression;
...
//                    printLog("avg = "+avg);
    ...
    private void uploadFile(Uri fileUri, final int document) {
        ...
        if (inputStream != null) {
            ...
            Bitmap scaled = ImageCompression.getScaledImage(original, 10);
            original.compress(Bitmap.CompressFormat.JPEG, 80, out);
            scaled.compress(Bitmap.CompressFormat.JPEG, 80, out1);
            finalOutputStream = ImageCompression.getCompressedImage(original, 102400);
            Bitmap scaledCompressed = BitmapFactory.decodeByteArray(finalOutputStream.toByteArray(), 0, finalOutputStream.size());
            scalledImage.setImageBitmap(scaledCompressed);
            ...
            printSize(scaledCompressed, "scalled compressed");
        }
    }
